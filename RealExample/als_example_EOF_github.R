### created at 13/07/2019 the sensitivity analysis real example for ALS data set by mony modeling EOF
...
library(survival)
cen_EOF = Surv(time = obs_surv$outcome, event = censor)
...
clean_dat$weight[clean_dat$stage > 1] = EOF_surv_weight$weight[clean_dat$stage[clean_dat$stage > 1]] * trt$ipw.weights
...
real_logit_EOF = list(temp1 = temp1, temp2 = temp2, temp3 = temp3, temp4 = temp4 ,
                      temp7 = temp7, temp8 = temp8)
setwd("~/Dropbox/Hsun/Research project/Causal Inference/ALS case study/QAL/code_github")
save(real_logit_EOF, file = "real_logit_EOF_all.rda")
