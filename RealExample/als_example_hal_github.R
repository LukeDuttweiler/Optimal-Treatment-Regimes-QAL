...
### combine this two part together
mod_cen89_num = fit_hal(X = rbind(tempdat_sub8[,idx2[c(1,2,3,7)]], tempdat_sub9[,idx2[c(1,2,3,7)]]),
                        Y = c(tempdat_sub8$cen, tempdat_sub9$cen),
                        family = "binomial",
                        yolo = FALSE)
...
preds_cen_num[tempdat_cen$id %in% c(211:270)] = predict(mod_cen89_num, new_data = rbind(tempdat_sub8, tempdat_sub9)[,idx2[c(1,2,3,7)]])
...
temp4 <- genoud(fn=smooth_opt2_als, index = c(3, 4, 5), nvars=4, default.domains=1, starting.values=rep(0, 4),
                max=TRUE, print.level=1, BFGS=FALSE, optim.method="Nelder-Mead", P9=0, unif.seed=1107, int.seed=0130)
mod4 = loss_indicator2_als(n = 481, eta = real_hal$temp4$par, clean_dat, upp = upp, time, target = "rmst", est_var = T, index = c(3, 4, 5))
