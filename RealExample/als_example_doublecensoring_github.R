...
setwd("F:/HSun/Research Project/Causal inference/ALSResearch/CeftriaxoneTrial/Erin data/data/20170316")
allcensoreddata <- read.sas7bdat("alldata.sas7bdat")
#alldata <- read.sas7bdat("alldata.sas7bdat")
basecensoreddata <- read.sas7bdat("baseline.sas7bdat")
...
#QOL = alssqol_average_total/max(alssqol_average_total)
...
setwd("~/Dropbox/Hsun/Research project/Causal Inference/ALS case study/QAL/code_github/RealExample")
...
clean_dat$weight = 1
trt$ipw.weights[trt$ipw.weights > 10] = 10
trt$ipw.weights[trt$ipw.weights < 0.1] = 0.1
clean_dat$weight[clean_dat$stage > 1] = EOF_surv_weight$weight[clean_dat$stage[clean_dat$stage > 1]] * trt$ipw.weights * cen_LTFU$ipw.weights 
...
#=====================================================================================================#
#=========== the example when only bmi and fvc are considered ========================================#
...
real_logit_double = list(temp1 = temp1, temp2 = temp2, temp3 = temp3, temp4 = temp4 ,
                      temp7 = temp7, temp8 = temp8)
setwd("~/Dropbox/Hsun/Research project/Causal Inference/ALS case study/QAL/code_github")
save(real_logit_double, file = "real_logit_double_all.rda")
...
### the opt case
sc_opt = surv_curve(real_logit_double$temp1$par, index)
...
pdf(paste0(path, "/real_survplot_double.pdf"), width = 5, height  = 5)
...
#======================================================================================================#
#=============== generate the survival curve for observed treatment regime ============================#
#======================================================================================================#

obs_trt = loss_indicator2_als(n = 481, eta = c(0, 0, 1), clean_dat, upp = upp, time, target = "rmst", est_var = T, index = c(3, 9))
sc_bmi = surv_curve(c(0, 0, 1), index = c(3,9))
plot(a[-length(a)], sc_bmi, type = "l", lty = 3, xlab = "Survival time (weeks)", ylab = "Surv.curve P(Surv > t)", lwd = 2)


km_surv = survfit(Surv(obs_surv$outcome, 1-cen$cen)~1)
plot(km_surv)
