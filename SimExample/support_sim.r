...
loss_indicator2 <- function(n, eta, dat, trt, cen, clean_dat, a, final_cen, upp = 30, time, target = "rmst", est_var = F, nuisance = "logit"){
  n_dat = clean_dat
  ...
  if(nuisance == "logit"){
    ...
    #==============================================================#
        ...
        if(flag_cen != 0 & flag_trt!=0){
          ...
          mod_trt = fit_hal(X = cbind(1, tempdat$x1, tempdat$x2)[tempdat$selvar_trt==1,] , Y = tempdat$trt[tempdat$selvar_trt==1], family = "binomial", yolo = FALSE, cv_select = F, lambda = under_lambda_trt)
          preds_trt = predict(mod_trt, new_data = cbind(1, tempdat$x1, tempdat$x2)[tempdat$selvar_trt==1,])
          ...
          mod_cen = fit_hal(X = cbind(1, tempdat$x1, tempdat$x2)[tempdat$selvar_cen==1,] , Y = tempdat$censor[tempdat$selvar_cen==1], family = "binomial", yolo = FALSE, cv_select = F, lambda = under_lambda_cen)
          preds_cen = predict(mod_cen, new_data = cbind(1, tempdat$x1, tempdat$x2)[tempdat$selvar_cen==1,])
...
optimal_sim2 <- function(n = 250, dat, nuisance = "logit", est = "IPW"){
  ...
  if(nuisance == "logit"){
    ...
    trt <- ipwtm_qal(exposure = trt, family = "binomial", link = "logit",
                     ...
                     id = ID, tstart = start, timevar = stage, type = "all",
                     data = clean_dat[clean_dat$stage > 1, ])
    cen <- ipwtm_qal(exposure = censor, family = "binomial", link = "logit",
                     numerator = NULL, denominator = ~ z1+z2,
                     id = ID, tstart = start, timevar = stage, type = "all",
    ...
    results = list(eta_opt = temp1$par, est1 = temp1$val, est2 = val, se = sqrt(mod1$var_est2))
  }
  else if(nuisance == "hal"){
    rate = 0.9
    #==============================================================#
...
data_gen_ch3_norm <- function(n,K, interval, eta_opt){
  stage = K/interval
...
### the function to generate data under the true optimal regime; used to calculated the true valuse of some causal estimand
data_gen_true_ch3_norm <- function(n,K, interval, eta, eta_opt, seed = 12345){
  y0 = -6
  y1 = -0.5
  y2 = -0.5
  y3 = -0.5
